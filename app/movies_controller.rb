...
def can_find_using_where_clause_and_be_sorted
  ...
  # release date descending
    sorted_list = Movie.all.sort_by do |m|
      -m.release_date
     end
    sorted_list.select do |m|
        m.release_date > 2002
     end
 end
...
def can_be_found_updated_and_saved
  ...
  a_movie = Movie.find_by(:title => "Awesome Flick")
  a_movie.title = "Even Awesomer Flick"
  a_movie.save
 end
...
def can_update_using_update_method
  ...
  a_movie = Movie.find_by(:title => "Wat?")
 a_movie.update(title: "Wat, huh?")
